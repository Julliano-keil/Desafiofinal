...
import '../data_repositories/database/db.dart';
...
class Zoom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final statePrefrs = Provider.of<PersonControler>(context);
    final settings = Provider.of<Settingscode>(context);
    final userid = statePrefrs.loggedUser?.id;
    final nameUser = statePrefrs.loggedUser?.storeName;

    final size = MediaQuery.of(context).size;
    return ChangeNotifierProvider<Zooncontroller>(
      create: (context) => Zooncontroller(userid: userid!),
      child: Consumer<Zooncontroller>(
        builder: (_, state, __) {
          return ZoomDrawer(
            controller: z,
            borderRadius: 50,
                  ...
                  children: <Widget>[
                      ...
                      children: [
                        Center(
                          child: CircleAvatar(
                            ...
                            radius: 59,
                            backgroundImage: state.imageuser != null
                                ? FileImage(File(state.imageuser!))
                    ...
                    userid == 1
                        ? ListTile(
                            ...
                            onTap: () async =>
                                await Get.to(const Registeredpeople()))
                        : Container(),
                    ListTile(
...
///
class Zooncontroller extends ChangeNotifier {
  ///
  Zooncontroller({required this.userid}) {
    unawaited(selectUserid(userid));
  }

  /// user id current
  int userid;

  /// controller table person
  ...
  Future<void> selectUserid(int userid) async {
    final list = await controller.selectUser(userid);
    imageuser = list[0].imageuser;
  }
}
